...
import (
	...
	autoscalingv1alpha1 "scaling-demo/pkg/apis/autoscaling/v1alpha1"
	clientset "scaling-demo/pkg/client/clientset/versioned"
	informers "scaling-demo/pkg/client/informers/externalversions/autoscaling/v1alpha1"
	listers "scaling-demo/pkg/client/listers/autoscaling/v1alpha1"
...
func (c *Controller) syncHandler(key string) error {
	...
	log.Printf("checkingggg podAutoscaler ........... %v, --  %v", podAutoscaler, podAutoscaler.Class())
	log.Printf("checkingggg podAutoscaler.Spec ........... %v", podAutoscaler.Spec)
	log.Printf("checkingggg podAutoscaler.Status ........... %v", podAutoscaler.Status)
	log.Printf("checkingggg podAutoscaler.Annotations ........... %s", podAutoscaler.Annotations)
	if podAutoscaler.Annotations["autoscaling.knative.dev/class"] == "yolo" {
		log.Print("got it yoloooooo ...........")

	} else {
		msg := fmt.Sprintf(MessageResourceExists, podAutoscaler.Name)
	...
	log.Printf("2: Got deployment %q, state %v", deployment.Name, deployment.Status)
	...
	replicas := recommendedScale(podAutoscaler)
	...
	if _, err := c.kubeclientset.AppsV1().Deployments(podAutoscaler.Namespace).Update(newdeployment); err != nil {
		log.Printf("Error updating Deployment %q: %v", podAutoscaler.Name, err)
	}
	// }
